...
<div class="tarjeta ancha">
    ...
    <p class="subtitulo">${seleccion.mensaje}</p>

    <c:choose>
        <c:when test="${seleccion.vacio}">
            <div class="vacio">
                <span class="vacio-rotulo">Sin destinos disponibles</span>
                <p>
                ...
                <a class="boton" href="<c:url value='/presupuesto'/>">Ajustar presupuesto</a>
            </div>
        </c:when>
            ...
            <c:forEach var="r" items="${seleccion.destinos}" varStatus="pos">
                <div class="ticket">
                    <div class="ticket-info">
                        <span class="ticket-sello">${pos.index + 1}</span>
                        <div>
                            <span class="ticket-ciudad">${r.ciudad.nombre}</span>
                            <span class="ticket-pais">${r.ciudad.pais}</span>
                        </div>
                    </div>

                    <div class="ticket-talon">
                        <span class="ticket-rotulo">Coincidencia</span>
                        <span class="ticket-puntaje">${r.porcentaje}%</span>
                        <span class="ticket-desglose">
                            Presupuesto ${r.porcentajePresupuesto}% ·
                            Gustos ${r.porcentajePreferencias}%
                        </span>
                    </div>
                </div>
            </c:forEach>
    ...
    <a class="volver" href="<c:url value='/resumen'/>">&larr; Volver al resumen</a>
</div>
</body>
</html>